...
function insertHTML(html) {
	...
	var pattern =/{\$(.+?)}/;	
	if(pattern.exec(html)){	
		html = "<IMG src='"+appName+"/eWebEditor/buttonimage/standard/label.gif'  Zlabel='"+html+"'>" ;
	}
...
// 取编辑器的内容(当保存或修改时会调用此方法)
function getHTML() {
	var html;
	if(bEditMode){
		html = eWebEditor.document.body.innerHTML;
		html = replaceLableByCode(html);
	}else{
...
function  replaceLableByEdit(code){
	//alert("byEdit = "+code)
	var pattern=/<!--{\$(.+?)}-->/gi;	
	...
	labelArr = code.match(pattern);	
	pic = "<IMG src='"+appName+"/eWebEditor/buttonimage/standard/label.gif'  Zlabel='{$0$1}'>" ;
	
	
   code=code.replace(pattern,pic);
	
	 return code;
	}
	
function  replaceLableByCode(code){
	//alert("byCode = "+code)
	if(code == null || code.length ==0)
	    code = eWebEditor.document.body.innerText;
		//alert(code)
	 var pattern = /<IMG(.[^\<]*)}">/gi;
	 picArr = code.match(pattern);
	...
	if(picArr !=null){
	   ...
	   for(var i = 0 ; i < picArr.length; i++) 
        {  
		   picstr = picArr[i].match(patt)[0];
		   picstr = "<!--" +picstr+"-->";
		   picstr = picstr.replace("$0","$");
	       code = code.replace(picArr[i], picstr);
...
function replaceByDelLabel(){
	  
	  if(eWebEditor.event.keyCode == 46 && isControlSelected("IMG")){
	    //alert("请检查标签数组！,当前代码为：/n"+eWebEditor.document.body.innerText);
		   oSelection = eWebEditor.document.selection.createRange();
			...
			var indexPos = in_array(labelArr,Zlabel);
			labelArr.splice(indexPos,1);  // 从数组中去除当前删除的标签
			//alert(" ="+ eWebEditor.document.body.innerHTML)
...
function setMode(NewMode){
	...
	//alert("NewMode="+NewMode+", sCurrMode="+sCurrMode);
	...
	if (NewMode!=sCurrMode){
		...
		case "CODE":
			...
			eWebEditor.document.open();
			eWebEditor.document.write(config.StyleEditorHeader);
			sBody = replaceLableByCode(sBody);
		...
		case "EDIT":		    
			...
			eWebEditor.document.write(config.StyleEditorHeader);
			bodyHTML = sBody;
			sBody = replaceLableByEdit(sBody);
			
			eWebEditor.document.body.innerHTML=sBody;			
			eWebEditor.document.body.contentEditable="true";
			eWebEditor.document.oncontextmenu=new Function("return showContextMenu(eWebEditor.event);");
			//eWebEditor.document.body.onkeydown = new Function("replaceByDelLabel()");
			eWebEditor.document.execCommand("2D-Position",true,true);
			eWebEditor.document.execCommand("MultipleSelection", true, true);
			eWebEditor.document.execCommand("LiveResize", true, true);
			eWebEditor.document.close();
