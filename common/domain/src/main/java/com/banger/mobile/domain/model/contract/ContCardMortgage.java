 ...
 * 担保主表
 */
public class ContCardMortgage extends BaseLnContract implements Serializable {
...
//    private String creditNo ;//统一社会信用代码
    ...
    private Date leaseBeginTime;//租赁期.开始
    private Date leaseEndTime;//租赁期.结束

    public ContCardMortgage(){

    }
    public ContCardMortgage(LnLoanInfo lnLoanInfo, List<LnPledge> lnPledgeList) {
        ...
        if(!CollectionUtils.isEmpty(lnPledgeList)){
            for(LnPledge bean : lnPledgeList ){
               ...
               /* if(StringUtils.isNotBlank(lnLoanInfo.getCusIdtypeId())){
                ...
                mortgageGoodsInfo.setGoodsName(bean.getGoods());
...
//        String str =lnLoanInfo.getCusName();
    ...
    public Date getLeaseBeginTime() {
        return leaseBeginTime;
    }

    public void setLeaseBeginTime(Date leaseBeginTime) {
        this.leaseBeginTime = leaseBeginTime;
    }
